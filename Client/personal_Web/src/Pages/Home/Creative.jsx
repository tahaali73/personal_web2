...
function Creative() {
  // Skill data with icon placeholders (you should replace with your actual icons)
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2
            className={`font-manrope font-semibold text-3xl sm:text-4xl md:text-[38px] text-[${largeTextcolor}] mb-4`}
          >
            Creative Skills
          </h2>
          <p
            className={`font-manrope font-normal text-base sm:text-lg md:text-xl text-[${mediumTextColor}] max-w-3xl mx-auto`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
          {skills.map((skill, index) => (
            <div
              key={index}
              className={`group bg-[${CardColor}] rounded-xl p-6 md:p-8 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-[${borderColor}] hover:border-[${secondaryColor}]`}
            >
              <div className="flex flex-col items-center">
                {/* Icon with animated background */}
                <div
                  className={`mb-6 p-4 bg-[${CardColor}] rounded-full group-hover:bg-[${secondaryColor}] transition-colors duration-300`}
                >
                ...
                <div className="text-center">
                  <h3
                    className={`font-manrope font-semibold text-xl md:text-[20px] text-[${largeTextcolor}] mb-3 group-hover:text-[${secondaryColor}] transition-colors`}
                  >
                    {skill.title}
                  </h3>
                  <p
                    className={`font-manrope font-normal text-sm md:text-base text-[${mediumTextColor}]`}
                  >
                    {skill.description}
                  </p>
                </div>
